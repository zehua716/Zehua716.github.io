...
假设 $\mathbf{H} _{AB} = K _B \cdot \left( \mathbf{R} _{BA} - \dfrac{\mathbf{t} _{BA} \cdot \mathbf{n} _A^\top}{d} \right) \cdot K _A^{-1}$
...
# **4. 立体视觉中的对极几何 $Géométrie$ épipolaire**
...
## **4.3 对极约束（Epipolar Constraint）**

**目标**：利用上述几何关系，形式化对极约束，建立 $m_1$ 和 $m_2$ 之间的数学关系。

**定义向量：**

<div>$$\left\{ \begin{aligned} &\mathbf{\underline{m}_1} \text{ 是从光心 } O_1 \text{ 到图像点 } \underline{m}_1 \text{ 的向量} \quad \overrightarrow{O_1 m_1}^{1}\\ &\mathbf{\underline{m}_2} \text{ 是从光心 } O_2 \text{ 到图像点 } \underline{m}_2 \text{ 的向量} \quad \overrightarrow{O_2 m_2}^{2}\\ &\mathbf{t_{12}} = \overrightarrow{O_1 O_2}^{1} \text{ 是两个相机光心之间的平移向量} \end{aligned} \right.$$</div>

**定义对极平面的法向量：**

<div>$$\left\{ \begin{aligned} &\text{在参考系 } 1 \text{ 中，} \overrightarrow{\mathbf{n}_1}^{1} = \underline{\mathbf{m}}_1 \times \mathbf{t}_{12} \\ &\text{在参考系 } 2 \text{ 中，} \overrightarrow{\mathbf{n}_2}^{2} = \mathbf{R}_{21} \overrightarrow{\mathbf{n}_1}^{1}，\text{ 其中 } \mathbf{R} \text{ 是相机间的旋转矩阵} \end{aligned} \right.$$</div>
...
2. **自由度**
   ...
   **自由度：**
   ...
   <div>$$\left\{ \begin{aligned} &\text{旋转矩阵 } \mathbf{R} \text{ 有 3 个自由度} \\ &\text{平移向量 } \mathbf{t} \text{ 的方向有 2 个自由度（因为尺度未知）} \\ &\text{因此，} \mathbf{E} \text{ 有 5 个自由度} \end{aligned} \right.$$</div>
...
## **4.6 本质和基础矩阵的估计（L’estimation de la matrice）**

- $\text{Caméra calibrée} \Rightarrow \text{estimation de la matrice essentielle (5 DDL)} \Rightarrow \text{l'algorithme des 5 correspondances}$

  相机已经校准 $\Rightarrow$ 本质矩阵 $\mathbf{E}$ 的估计（5 个自由度）$\Rightarrow$ 5 点对应算法

- $\text{Caméra non-calibrée} \Rightarrow \text{estimation de la matrice fondamentale (7 DDL)} \Rightarrow \text{l'algorithme des 7 correspondances}$

  相机未校准 $\Rightarrow$ 基础矩阵 $\mathbf{F}$ 的估计（7 个自由度）$\Rightarrow$ 7 点对应算法

- $\text{Solution} \Rightarrow \text{l'algorithme 8 correspondances} \Rightarrow \text{omettre intentionnellement la contrainte } \det(\mathbf{F}) = 0$
